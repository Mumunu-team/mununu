...
| `MUNUNU_BDD_FIXPOINT_NODES` | mununu#553 — Exact-symbolic engine **fixpoint live-node budget**, the DETERMINISTIC guard on the μ/ν fixpoint (`abstained on the fixpoint NODE budget`). Default `1 << 21` (2 M), applied as `min(cap, node_budget)` so it can never exceed the arena — `0` means "fall back to the arena-derived budget", NOT unbounded. To raise the real ceiling, raise `MUNUNU_BDD_ARENA_NODES` with it. Distinct from `MUNUNU_BDD_ARENA_NODES`'s budget, which guards the bit-blasting ops, not the fixpoint. |
| `MUNUNU_BDD_REPORT_PEAK` | mununu#553 — set (any value) to print the exact fixpoint's PEAK live-node count, one line per evaluation: `[mununu#553] exact fixpoint: peak <N> live BDD nodes in <K> iteration(s), budget <B>`. How to size `MUNUNU_BDD_FIXPOINT_NODES` for a cone that abstains. Counts in ~2^16 chunks, so read peaks to the nearest 65536 (`peak 1` = under one chunk). |
