...
| `MUNUNU_BDD_VAR_ORDER` | mununu#553 follow-up — exact-engine **BDD variable order**: `cell-major` (DEFAULT), `interleaved` (bit-position-major), `deps` (interleave only cells some expression reads together). Order never changes a verdict (ROBDDs are canonical) — only size and time. Measured: a symbolic-shift-amount cone is 65,537 nodes cell-major and **1** interleaved; the whole lib suite is 2.6× faster interleaved. NOT flipped by default because one consumer block regresses 9.4× in wall time. See [`docs/design/bdd-variable-ordering.md`](docs/design/bdd-variable-ordering.md). |
...
| `MUNUNU_BDD_ORDER_DEBUG` | mununu#553 follow-up — print the variable-order cluster structure (`groups` / `singletons` / largest sizes). Diagnostic for `MUNUNU_BDD_VAR_ORDER=deps`. |
