...
- **Pin exact tags.** Never use `FROM ubuntu:latest` or `FROM rust:latest`. Use `FROM rust:1.95-slim-trixie` (the dev image's base; trixie for libz3 ≥ 4.13.3, see `docker/Dockerfile.dev`). Builds must stay reproducible. If you update the tutorial Dockerfile's `ubuntu:24.04`, pin it to a digest or full minor tag.
